...
def _override_docker_command(service, command, file, orig_file=None):
    ...
    docker_config = {"services": {service: {"command": command}}}
    if docker_compose_file_version is not None:
        docker_config["version"] = str(docker_compose_file_version)
...
def write_code_workspace_file(c, cw_path=None):
    ...
    for subrepo in SRC_PATH.glob("*"):
        ...
        if (subrepo / ".git").exists() and subrepo.name != "odoo":
            if subrepo.name in repo_analysis:
...
def install(
    ...
    if not (modules or core or extra or private or enterprise):
        ...
        modules = cur_module
    cmd = DOCKER_COMPOSE_CMD + f" run --rm odoo addons {mode}"
    if core:
        cmd += " --core"
    if extra:
        cmd += " --extra"
...
@task(
    help={
        "all_addons": "Whether to update all addons, not just the ones with changed "
        "checksums. Default: False",
    },
)
def update(
    c,
    ...
    """Run Click Odoo Update

    By default, updates changed checksum modules only.
    """
    cmd = DOCKER_COMPOSE_CMD + " run --rm odoo click-odoo-update"
    if all_addons:
        cmd += " --update-all"
    with c.cd(str(PROJECT_ROOT)):
        c.run(
...
def remote(
    ...
    """Execute marabunta migrate script."""
    if backup:
        cmd = "python3 rpc_db.py -b"
        with c.cd(str(PROJECT_ROOT)):
            c.run(cmd, pty=True)
    if restore and not environment:
        cmd = "python3 rpc_db.py --choose"
        with c.cd(str(PROJECT_ROOT)):
            result = c.run(cmd, pty=True)

            # Find all occurrences of text inside square brackets
            matches = re.findall(r"\[([^\[\]]+)\]", result.stdout)

            if matches:
                # Get the last match (last valid occurrence inside [])
                environment = matches[-1]
            else:
                environment = None  # Or handle as needed (e.g., raise an error)

        _logger.info(f"Selected remote: {environment}.")
    if restore:
        # TODO: consider making global
        f = open(str(PROJECT_ROOT) + "/.remotes.json")
        data = json.load(f)
        HOSTS = data
        db_name = HOSTS.get(environment).get("dbname")
        # Restore via click-odoo
        _logger.info("Restoring DB %s to %s" % (db_name, destination_db))
        _run = f"{DOCKER_COMPOSE_CMD} run --rm -l traefik.enable=false -v"
        _run += f'"$(realpath ../{db_name}.zip)":/mnt/{db_name}.zip odoo'
        with c.cd(str(PROJECT_ROOT)):
            c.run(
                f"{_run} click-odoo-restoredb {destination_db}"
                f" /mnt/{db_name}.zip --force",
                env=UID_ENV,
                pty=True,
            )
        # Always restart after restoring a on devel
        if destination_db == "devel":
            start(c)
